...
def _parse_duration(duration: str) -> int | None:
    ...
    patterns = [
        (r"^(\d+)\s*(seconds|second|secs|sec)$", 1),
        (r"^(\d+)\s*(minutes|minute|mins|min)$", 60),
        (r"^(\d+)\s*(hours|hour|hrs|hr)$", 3600),
        (r"^(\d+)\s*(days|day|d)$", 86400),
    ]
    ...
    time_match = re.match(r"^(\d+):(\d+)(?:\s*(am|pm))?$", duration)
    if time_match:
...
def timer(
    ...
    duration_str = parameters.get("duration", "").strip()
    message      = parameters.get("message", "Timer complete!").strip()

    if not duration_str:
        return "I need a duration to set a timer."

    seconds = _parse_duration(duration_str)
    if seconds is None or seconds <= 0:
        return "I couldn't understand that duration. Use values like '10 minutes', '30 seconds', or '2 hours'."

    target_dt = datetime.now() + timedelta(seconds=seconds)
    os_name   = _get_os()
    safe_msg  = _sanitise(message)
    task_name = f"JARVISTimer_{target_dt.strftime('%Y%m%d_%H%M%S')}"

    try:
        script_path = _write_notify_script(task_name, safe_msg, os_name)
    except Exception as e:
        return f"Could not prepare the timer script: {e}"
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        print(f"[Timer] ❌ Scheduling exception: {e}")
        return "Something went wrong while scheduling the timer."

    if not job_id:
        return "I couldn't register the timer with the system scheduler."

    if player:
        player.write_log(f"[Timer] ⏱️ {duration_str} — {safe_msg[:40]}")
        if hasattr(player, "add_widget"):
            player.add_widget(task_name, "Timer", safe_msg, duration=seconds, widget_type="timer")
