...
class _VisionSession:
    ...
    async def _send_loop(self) -> None:
        while True:
            try:
                image_bytes, mime_type, user_text = await asyncio.wait_for(self._out_queue.get(), timeout=30.0)
            ...
            try:
                ...
                print(f"[Vision] ⚠️  Send timeout after 20s")
                self._session = None
            except Exception as e:
    ...
    async def _recv_loop(self) -> None:
        ...
        try:
            async for response in self._session.receive():
                if response.data:
                    try:
                        await asyncio.wait_for(self._audio_in.put(response.data), timeout=5.0)
                    except asyncio.TimeoutError:
                        print(f"[Vision] ⚠️  Audio queue timeout")
    ...
    async def _play_loop(self) -> None:
        ...
        try:
            while True:
                try:
                    chunk = await asyncio.wait_for(self._audio_in.get(), timeout=5.0)
